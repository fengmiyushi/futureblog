...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm(`Are you sure you want to delete the dictionary item(s) with ID(s) "${dictIds}"?`).then(function() {
    ...
    handleRefreshCache() {
      refreshCache().then(() => {
        this.$modal.msgSuccess("Cache refreshed successfully");
      });
    }
  }
};
